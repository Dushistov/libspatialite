...
/* testing a 'raw' GPKG table */
	...
	"ST_MinM(geom), ST_MaxM(geom), ST_GeometryType(geom) FROM %s", table);
	  ...
	  if (ret == SQLITE_ROW)
	    {
			...
			{
				fprintf(stderr, "Unexpected ST_IsValidGPB: not an Integer\n");
				goto stop;
			}
			if (is_null)
			{
			...
			{
				fprintf(stderr, "Unexpected result (NULL) ST_IsValidGPB=%d\n", sqlite3_column_int(stmt, 1));
				goto stop;
			}
			continue;
		}
		else
		{
			...
			{
				fprintf(stderr, "Unexpected result ST_IsValidGPB=%d\n", sqlite3_column_int(stmt, 1));
				goto stop;
			}
		}
			...
			{
				fprintf(stderr, "Unexpected ST_IsMeasured: not an Integer\n");
				goto stop;
			}
			...
			{
				fprintf(stderr, "Unexpected ST_MinZ: not a Double\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 10) != SQLITE_FLOAT)
			{
				fprintf(stderr, "Unexpected ST_MaxZ: not a Double\n");
				goto stop;
			}
			}
			else
			...
			{
				fprintf(stderr, "Unexpected ST_MinM: not a Double\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 12) != SQLITE_FLOAT)
			{
				fprintf(stderr, "Unexpected ST_MaxM: not a Double\n");
				goto stop;
			}
			}
			else
			...
			{
				fprintf(stderr, "Unexpected ST_GeometryType: not TEXT\n");
				goto stop;
			}
		}
	  else
...
/* testing a VirtualGPKG table */
	...
	"last_name, value1, value2 FROM vgpkg_%s", table);
	}
	else
	{
	  ...
	  if (ret == SQLITE_ROW)
	    {
			...
			{
				fprintf(stderr, "Unexpected ROWID: not an INTEGER\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 1) != SQLITE_TEXT)
			{
				fprintf(stderr, "Unexpected \"first_name\": not a TEXT string\n");
				goto stop;
			}
			...
			if (sqlite3_column_type(stmt, 3) != SQLITE_FLOAT)
			{
				fprintf(stderr, "Unexpected \"first_name\": not a DOUBLE\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 4) != SQLITE_FLOAT)
			{
			...
			if (sqlite3_column_type(stmt, 1) != SQLITE_TEXT)
			{
				fprintf(stderr, "Unexpected ST_AsText: not a TEXT string\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 2) != SQLITE_INTEGER)
			{
				fprintf(stderr, "Unexpected \"id\": not an INTEGER\n");
				goto stop;
			}
			if (sqlite3_column_type(stmt, 3) != SQLITE_TEXT)
			{
				fprintf(stderr, "Unexpected \"name\": not a TEXT string\n");
				goto stop;
			}
		}
		}
...
/* testing VirtualGPKG insert/update/delete */
    ...
    if (ret != SQLITE_OK)
      ...
      sql = "INSERT INTO vgpkg_test_pk (first_name, last_name, value1, value2, geom) "
    ...
    if (ret != SQLITE_OK)
      ...
      sql = "UPDATE vgpkg_test_pk SET value1 = 13.4 "
      "WHERE ROWID = 3";
    ret = sqlite3_exec (handle, sql, NULL, NULL, &sql_err);
    if (ret != SQLITE_OK)
      {
...
/* directly testing GPKG */
    ...
    ret = sqlite3_open_v2 ("./copy-gpkg_test.gpkg", &db_handle, SQLITE_OPEN_READWRITE, NULL);
    if (ret != SQLITE_OK)
      {
